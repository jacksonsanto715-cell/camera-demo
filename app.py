...
# Usa caminhos absolutos a partir deste arquivo. Isso evita depender do diretório
# de trabalho escolhido pelo provedor de hospedagem (por exemplo, Render).
...
def login_required(function):
    ...
    def decorated_function(*args, **kwargs):
        if not session.get("authenticated") or not session_user_is_active():
            session.clear()
...
def session_user_is_active():
    """Invalida o acesso de uma conta removida mesmo se ela ainda tinha sessão."""
    ...
    try:
        ...
        return bool(user and user.get("active") and user.get("id") == user_id)
    except Exception:
        app.logger.exception("Não foi possível validar a sessão do usuário")
        return False
...
def render_named_capture_page(page_key):
    """
    Cada página pública possui seu próprio template:
        templates/pages/<page_key>.html

    Isso mantém o visual de cada usuário completamente independente
    do template principal (templates/index.html).
    """
    template_name = f"pages/{page_key}.html"
    template_path = BASE_DIR / "templates" / "pages" / f"{page_key}.html"
...
@app.get("/<page_key>")
def named_page(page_key):
    """
    URL pública independente de cada usuário:
        /1
        /2
        /3
        ...

    O identificador também determina o template visual.
    """
    try:
        page_key = normalize_page_key(page_key)
        return render_named_capture_page(page_key)
    except ValueError:
        return "Página não encontrada.", 404


# Compatibilidade com páginas antigas no formato /nome.html.
@app.get("/<page_key>.html")
def named_index(page_key):
    try:
        page_key = normalize_page_key(page_key)

        # Se existir um template independente, ele tem prioridade.
        template_path = BASE_DIR / "templates" / "pages" / f"{page_key}.html"
        if template_path.is_file():
            return render_named_capture_page(page_key)

        return render_capture_page(page_key)
    except ValueError:
        return "Página não encontrada.", 404
...
def api_delete_user(user_id):
    try:
        user = get_dashboard_user_by_id(user_id)
        if not user:
            return jsonify(success=False, error="Usuário não encontrado."), 404
        if user.get("is_admin"):
            return jsonify(success=False, error="A conta de administrador não pode ser removida por aqui."), 400
        delete_dashboard_user(user_id)
        return jsonify(success=True)
    except Exception:
        app.logger.exception("Erro ao apagar usuário")
        return jsonify(success=False, error="Não foi possível apagar o usuário."), 500
